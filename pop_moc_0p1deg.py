...
# Define MOC coordinates                               #定义moc四维变量
...
z_bot=z_w.values
z_bot=z_w.values+dz.values                       #每一层的top和bottom               dz厚度
z_top=z_w.values
...
dzt = util.pbc_dzt(dz,kmt,ht,z_w_bot,mval)      #每层厚度，T网格上最深的是哪一层，T网格海洋深度，depth from surface to bottom of layer 从表层到底层的深度
